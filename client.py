...
def close(client):
    ...
    send_message(client, '{} off '.format(client.client_name), ALL_CLIENTS)
    client.sock.close()
    os._exit(0)
        ...
        while True:
            ...
            if message:
                if self.messages:
                    ...
                    print('Message receive {}'.format(msg))
                    add_message(self.messages, '{}: {}'.format(msg[SENDER], msg[MESSAGE_TEXT]))
            else:
                add_message(self.messages, 'Connection lost')
                close(self)
...
class Client:
    ...
    def start(self):
        add_message(self.messages, 'Connect to {}:{}'.format(self.host, self.port))
        ...
        add_message(self.messages, 'Successful connection')
        return receive
    ...
    def send(self, text_input, to_input):
        ...
        if message == 'EXIT':
            ...
            add_message(self.messages, 'You > {}: {}'.format(to, message))
...
def main(host, port, client_name):
    ...
    btn_send = tkinter.Button(text='Sens', command=lambda: client.send(text_input, to_input))
